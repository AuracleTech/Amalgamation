mod args;
use args::Args;
use image::{ io::Reader, DynamicImage, ImageFormat, imageops::Lanczos3, GenericImageView, ImageError };
...
fn main() -> Result<(), ImageDataErrors> {
    ...
    let (image_1, image_2) = standardize_dimensions(image_1, image_2);
...
fn standardize_dimensions(image_1: DynamicImage, image_2: DynamicImage) -> (DynamicImage, DynamicImage) {
    let (width, height) = get_smallest_dimensions(image_1.dimensions(), image_2.dimensions());
    if image_2.dimensions() == (width, height) {
        (image_1.resize_exact(width, height, Lanczos3), image_2)
    } else {
        (image_1, image_2.resize_exact(width, height, Lanczos3))
    }
}
...
fn alternate_pixels(vec_1: Vec<u8>, vec_2: Vec<u8>) -> Vec<u8> {
    ...
    while i < vec_1.len() {
        if i % 6 == 0 {
            combined_data.splice(i..=i + 2, set_rgb(&vec_1, i, i + 2));
        } else {
            combined_data.splice(i..=i + 2, set_rgb(&vec_2, i, i + 2));
        }
        i += 3;
    }
    combined_data
...
fn set_rgb(vec: &Vec<u8>, start: usize, end: usize) -> Vec<u8> {
